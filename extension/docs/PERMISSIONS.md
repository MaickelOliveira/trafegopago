...
**Uso**: injetar dois content scripts. (1) `content-script.ts`, mundo ISOLADO padrão, observa via seletores DOM se a tela de conversas está visível/há QR Code/está carregando, faz o relay das mensagens que (2) `main-world.ts` reporta, e busca respostas pendentes da IA por polling pra mandar `main-world.ts` enviar. `main-world.ts` roda no mundo PRINCIPAL da página (`"world": "MAIN"` em `manifest.json`), compartilhando o contexto JS da própria aplicação do WhatsApp Web, e usa a lib `@wppconnect/wa-js` pra ler nome de contato, telefone, texto de mensagens novas e contexto de anúncio de origem — dado que não é exposto de forma confiável no DOM visível — e, quando o Agente IA está ativo, ENVIAR a resposta automaticamente (`chat.sendTextMessage`). Tudo isso pra criar/atualizar leads no CRM do usuário e (se aplicável) responder automaticamente, mesma finalidade de uma integração de WhatsApp normal (ver `docs/PRIVACY.md`).

**⚠️ Nota de risco**: rodar no mundo principal e ler/escrever no estado interno da aplicação via `@wppconnect/wa-js` é uma técnica não-oficial, fora dos termos de uso do WhatsApp — mesma categoria de risco de Evolution API/WPPConnect, mas recaindo sobre o número pessoal do cliente. `main-world.ts` chama só UMA função de escrita do pacote (`chat.sendTextMessage`, e só quando o Agente IA está ativo na conexão) — nunca nenhuma outra automação (grupos, mídia, reações, etc.), apesar do pacote trazer bem mais capacidade que isso. Envio automatizado é o padrão que sistemas de detecção de bot mais observam — risco mais sério que só leitura. Ver disclosure completo em `docs/PRIVACY.md`.

**Por que é o mínimo necessário**: restrito exatamente ao domínio do WhatsApp Web — não há `<all_urls>` nem wildcard amplo. Nenhum dos dois scripts lê cookies, `localStorage`, `IndexedDB`, nenhum abre uma conversa pra ler o histórico completo, e nenhum lê/envia mídia ou mensagem de grupo (ver `src/main-world.ts` e `src/whatsapp-dom-adapter.ts`).

## `host_permissions` — domínio da plataforma

**Uso**: `fetch()` do service worker, do popup e do content script para os endpoints `/api/integrations/whatsapp-extension/*` (claim do código, heartbeat, envio de mensagens novas pra criação de lead, e busca/confirmação de respostas pendentes da IA).
