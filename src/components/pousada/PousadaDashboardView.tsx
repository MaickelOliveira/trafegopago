...
const STATUS_CLASS: Record<Reserva["status"], string> = {
...
export function PousadaDashboardView({ clientId, role }: { clientId: string; role: "manager" | "client" }) {
  ...
  // Carregamento inicial da tela; `load` também é reutilizado após salvar.
  ...
  return (
    <div>
      ...
      {/* Filtro de período aplicado à lista única de todos os serviços abaixo */}
      <div className="rounded-xl border border-slate-200 bg-white p-4 space-y-3">
        <div>
          <p className="text-sm font-semibold text-slate-700">Filtrar todos os serviços por data</p>
          <p className="mt-0.5 text-xs text-slate-400">O período escolhido atualiza os totais, a busca e a lista completa de reservas.</p>
        </div>
      ...
      {/* Lista consolidada: hospedagem, day use, almoço e qualquer outro
          serviço configurado, todos obedecendo ao mesmo filtro de data. */}
      {!buscaAtiva && (
        <div className="space-y-3">
          <div className="flex flex-wrap items-end justify-between gap-2">
            <div>
              <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Todos os serviços no período</h2>
              <p className="mt-0.5 text-xs text-slate-400">{upcoming.length} reserva{upcoming.length === 1 ? "" : "s"} encontrada{upcoming.length === 1 ? "" : "s"}</p>
            </div>
          </div>

          {upcoming.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-200 bg-white px-5 py-10 text-center text-sm text-slate-400">
              Nenhum serviço encontrado nas datas selecionadas.
            </div>
          ) : (
              ...
              {upcoming.map((r) => {
                ...
                return (
                  <Link
                    key={r.id}
                    href={`${base}/reservas/${r.id}`}
                    className="flex flex-col gap-3 px-5 py-4 transition hover:bg-amber-50/40 sm:flex-row sm:items-center sm:justify-between"
                  >
                    <div className="min-w-0">
                      ...
                      <p className="mt-1.5 truncate font-medium text-slate-800">{r.responsavel.nome}</p>
                      <p className="mt-0.5 text-xs text-slate-500">
                        {formatDataComDiaSemana(r.data)}{r.hora ? ` às ${r.hora}` : ""} · {r.pessoas.length} pessoa{r.pessoas.length === 1 ? "" : "s"}
                      </p>
                    </div>
                    <div className="grid shrink-0 grid-cols-3 gap-4 text-left sm:text-right">
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-slate-400">Total</p>
                        <p className="text-sm font-medium text-slate-700">{fmt(r.valorTotal)}</p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-slate-400">Pago</p>
                        <p className="text-sm font-medium text-green-600">{fmt(r.valorPago)}</p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-slate-400">Falta</p>
                        <p className="text-sm font-medium text-amber-600">{fmt(r.faltaPagar)}</p>
                      </div>
                    </div>
                  </Link>
                );
