...
export async function runGeminiAgent(
  ...
  // assim. Aqui a saudação é enviada de forma determinística por código
  // (sem chamar o Gemini) só na primeira mensagem de cada "dia de
  // conversa" — vira dia novo (meia-noite), a próxima mensagem do cliente
  // recebe a saudação de novo, mesmo que tenham se passado só alguns
  // minutos desde a última troca. Fora isso (mesmo dia da última
  ...
  if (agentCfg.greetingOnlyMode) {
    ...
    console.log(`[gemini-agent] greetingOnlyMode — saudação enviada (dia novo) — clientId=${clientId} phone=${phone}`);
